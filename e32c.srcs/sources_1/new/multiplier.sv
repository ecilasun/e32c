...
module singedunsignedmultiplier(
    ...
    output logic [31:0] product = 32'd0 );	// result
...
always_ff @(posedge clk) begin
	if (~resetn) begin
		...
		if (start) begin
			...
			endcase
			// The DSP multiplier latency is set to 6 clocks in the IP
			// Be safe and grab output on 7th
			n <= 7;
			busy <= 1'b1;
